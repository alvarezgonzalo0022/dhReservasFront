...
const Home = () => {
  ...
  // const [sacarFiltros, setSacarFiltros] = useState(false);
  const [dataAutosRandom, setDataAutosRandom] = useState([]);
  ...
  const newDataRandom = dataAutosRandom?.map((item) => {
    const sortedImages = item.imagenes.sort((a, b) => {
      const aIndex = parseInt(a.titulo[a.titulo.length - 1], 10);
      const bIndex = parseInt(b.titulo[b.titulo.length - 1], 10);
      return aIndex - bIndex;
  ...
  return (
    <section style={{ width: "100%" }}>
      {categoryList.length === 0 ? (
        ...
        <section>
          <Buscador />
          <div className="listCardCategoria">
            <h2 className="tituloTipo">Buscar por tipo de vehiculo</h2>
            {categoryList.map((auto) => (
              <div key={auto.idCategoria} className="cardCate">
                <CardCategoria
                  imagen={auto.urlImagen}
                  categoria={auto.titulo}
                  identificador={auto.idCategoria}
                />
              </div>
            ))}
          </div>
          <div className="listCard">
            <h2 className="tituloRecomen">
              Recomendaciones{" "}
              {quitarFiltro ? (
                <span onClick={quitarfiltros}>
                  <span> - </span>
                  <span className="eliminarFiltros">Eliminar filtros</span>
                </span>
              ) : (
                ""
              )}
            ...
            {dataAutos.length === 0 ? (
              ...
              >
                <p style={{ fontSize: "18px", fontWeight: "500" }}>
                  Cargando los autos, espere un momento...
                </p>
                <div className="bulb">
                  <svg
                    width="60pt"
            ...
            {login ? (
              !quitarFiltro ? (
                newData?.map((auto) => (
                  <Card
                    key={auto.idProducto}
                    identificador={auto.idProducto}
                    imagen={auto.imagenes[0].image}
                    categoria={auto.categoria.titulo}
                    nombre={auto.titulo}
                    ubicacion={auto.ciudad.nombre}
                    descripcion={auto.descripcion}
                  />
                ))
              ) : listAutosUbi.length === 0 || listAutosUbi[0].length === 0 ? (
                <div
                  style={{
                    width: "100%",
                    display: "flex",
