...
import "./ModificarProductoAdminStyles.css";
import CardModificar from "./CardModificar";
import ProductoAModificar from "./ProductoAModificar";

const ModificarProductoAdmin = () => {
  ...
  useEffect(() => {
    ...
    if (start && hasMore) {
      if (document.getElementById("observador")) {
  ...
  function fetchSubmitProductos() {
    ...
    setHref(
      `https://dhreservas-bd190ac8106b.herokuapp.com:8080/productos/filtrarProductosPaginados?start=null` +
        ...
        "&categoria=" +
        dataModificarProducto.categoria +
        "&ciudad=" +
        dataModificarProducto.ciudad
    );
    try {
      fetch(
        `https://dhreservas-bd190ac8106b.herokuapp.com:8080/productos/filtrarProductosPaginados?start=null` +
  ...
  const [dataModificarProducto, setDataModificarProducto] = useState({
    titulo: null,
    categoria: null,
    ciudad: null,
  });

  const [idProductoModificar, setIdProductoModificar] = useState(null);
  // console.log(idProductoModificar)
  const [cambiarModificar, setCambiarModificar] = useState(false);

  function confirmarModificacion() {
    if (cambiarModificar) {
      setCambiarModificar(false);
    } else {
      setCambiarModificar(true);
    }
  }
  ...
  return cambiarModificar ? (
    <div style={{ position: "relative" }}>
      <ProductoAModificar idProductoModificarNum={idProductoModificar} />
      <button className="cancelarModificacion" onClick={confirmarModificacion}>
        <span className="material-symbols-outlined">close</span>
      </button>
    </div>
  ) : (
    <div className="modificarProductoAdmin">
      <form className="filtrarModificados" onSubmit={handleSubmit}>
        <input
          className="inputsCrearModificados"
          name="tituloDeProducto"
          placeholder="Ingresar titulo de producto"
          onChange={(e) =>
            setDataModificarProducto({
              ...dataModificarProducto,
        ...
        {dataAutos?.length === 0 ? (
          <div>
            <p style={{ fontSize: "18px" }}>
              Cargando los productos o no hay productos creados.
            </p>
            <div className="bulb">
              <svg
                width="60pt"
          ...
          newData?.map((auto) => (
            <div
              ...
              className="cardProductosModificar"
              key={auto.idProducto}
            >
              <CardModificar autoObjeto={auto} key={auto.idProducto} />
              <button
                className="modificarProducto"
                onClick={() => {
                  confirmarModificacion();
                  setIdProductoModificar(auto.idProducto);
                }}
              >
                Modificar
              </button>
      ...
      {dataAutos?.length === 0 ? (
        ...
        <div id="observador" style={{ textAlign: "center" }}>
          No se encuentran mas productos
        </div>
      )}
    </div>
  );
